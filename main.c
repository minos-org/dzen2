...
set_docking_ewmh_info(Display *dpy, Window w, int dock) {
	...
	unsigned long strut_width = 0;
	unsigned long strut_start = 0;
	unsigned long strut_end = 0;
	...
	/* Do we want to dock dzen? */
	if( dock != DOCK_NONE ) {
		/* Yes, we do. Calculate the strut area: */
...
#endif
	if(wa.y - si.y == 0) {
			strut_width = si.y + wa.height;
			strut_start = wa.x;
			strut_end   = wa.x + wa.width - 1;
	}
		...
		/* Adjust strut value if there is a larger screen */ 
			strut_width = max_height - (si.height + si.y) + wa.height;
#else
			strut_width = wa.height;
#endif
			strut_start = wa.x;
			strut_end   = wa.x + wa.width - 1;
	}
		...
		switch( dock )
		{
			...
			case DOCK_BOTTOM:
			default:
				strut[3]  = strut_s[3] = strut_width;
				strut[10] = strut_start;
				strut[11] = strut_end;
				break;
	}


		/* Finally, set some helpful X properties: */
	...
	/* Even if we don't want to dock dzen, we still set the strut area (disable it in that case). */
	XChangeProperty(
			dpy,
			w,
...
x_create_windows(int use_ewmh_dock) {
	...
	/* window attributes */
	wa.override_redirect = use_ewmh_dock != DOCK_NONE ? 0 : 1;
...
main(int argc, char *argv[]) {
	...
	for(i = 1; i < argc; i++)
		if(!strncmp(argv[i], "-l", 3)){
			...
			use_ewmh_dock = DOCK_BOTTOM;

			if(i+1 < argc) {
				if(*argv[i+1] == 't') {
					use_ewmh_dock = DOCK_TOP;
					i++;
				}
				else if(*argv[i+1] == 'b') {
					/* Set to DOCK_BOTTOM by default. */
					i++;
				}
			}
		}
			...
			eprint("usage: dzen2 [-v] [-p [seconds]] [-m [v|h]] [-ta <l|c|r>] [-sa <l|c|r>]\n"
				   ...
				   "             [-geometry <geometry string>] [-expand <left|right>] [-dock [t|b]]\n"
